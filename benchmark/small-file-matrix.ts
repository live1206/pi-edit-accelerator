...
type Backend = "built-in" | "typescript";
...
function parseArgs(args: string[]): Options {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === "--worker-backend") {
      ...
      if (value !== "built-in" && value !== "typescript") {
...
async function runWorker(options: Options & { workerBackend: Backend }): Promise<WorkerReport> {
  ...
  return { backend: options.workerBackend, results };
}
...
async function runCoordinator(options: Options): Promise<void> {
  ...
  const comparisons = [];
  for (const typescript of summarized.filter((result) => result.backend === "typescript")) {
    const builtIn = summarized.find((result) =>
      result.backend === "built-in" &&
      result.bucket === typescript.bucket &&
      result.scenario === typescript.scenario &&
      result.lifecycle === typescript.lifecycle,
    );
    if (!builtIn) throw new Error("Missing built-in comparison case");
    comparisons.push({
      bucket: typescript.bucket,
      fixtureBytes: typescript.fixtureBytes,
      scenario: typescript.scenario,
      editCount: typescript.editCount,
