...
const input: EditToolInput = {
  path: "large.txt",
  edits: [{ oldText: "LAST_MARKER", newText: "LAST_MARKER_EXPANDED" }],
};

async function measure(suffix: boolean): Promise<Sample> {
  const directory = await mkdtemp(join(tmpdir(), "pi-edit-suffix-"));
  ...
  try {
    ...
    if (!prepared.suffixWrite) throw new Error("Fixture is not eligible for suffix writes");
    const executionPlan = suffix ? prepared : { ...prepared, suffixWrite: undefined };
...
const suffixReference = await measure(true);
deepStrictEqual(suffixReference.result, fullReference.result);
strictEqual(suffixReference.content, fullReference.content);
...
const suffixWriteSamples: number[] = [];
for (let index = 0; index < 20; index++) {
  if (index % 2 === 0) {
    fullWriteSamples.push((await measure(false)).wallTimeMs);
    suffixWriteSamples.push((await measure(true)).wallTimeMs);
  } else {
    suffixWriteSamples.push((await measure(true)).wallTimeMs);
    fullWriteSamples.push((await measure(false)).wallTimeMs);
  }
  ...
  `${JSON.stringify(
    {
      ...
      firstChangedByte: Buffer.byteLength(makeFixture()) - Buffer.byteLength("LAST_MARKER\n"),
      fullWrite: summarize(fullWriteSamples),
      suffixWrite: summarize(suffixWriteSamples),
    },
    null,
    2,
  )}\n`,
